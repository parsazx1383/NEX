    ...
    with ThreadPoolExecutor(max_workers=50) as executor:
        for i in range(count):
            for func in functions:
                if executor.submit(func, phone).result():
                    success_count += 1
                print(f"{g}[+] Sending request {i*len(functions)+functions.index(func)+1}/{total_requests}... {'Success' if executor.submit(func, phone).result() else 'Failed'}")
